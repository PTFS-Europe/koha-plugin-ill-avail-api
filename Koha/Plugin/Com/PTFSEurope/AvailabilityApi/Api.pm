...
my $base_url = "https://printdocs-staff.koha-ptfs.co.uk/"; #TODO: Put this in a config value
...
sub get_auth_token {
    ...
    my $userid        = $plugin_config->{ill_avail_api_userid};
    my $password      = $plugin_config->{ill_avail_api_password};
...
__BODY__
    ...
    my $auth_body = parse_response(
        $auth_response,
        { c => $c, err_code => 500, error => 'Unable to authenticate to Koha API' }
    );
    ...
    if ( !exists $auth_body->{AuthToken} ) {
        return_error(
            $c,
            500,
            'Unable to authenticate to Koha API: ' . $auth_response->decoded_content
        );
    }
