package Koha::Plugin::Com::PTFSEurope::AvailabilityApi;
...
our $metadata = {
    name            => 'ILL availability - API',
    author          => 'PTFS Europe',
    date_authored   => '2024-08-09',
    date_updated    => "2024-08-09",
    minimum_version => '23.11.00.000',
    maximum_version => undef,
    version         => $VERSION,
    description     => 'This plugin provides ILL availability searching using the Koha API'
};
...
sub get_name {
    my ($self) = @_;
    return $self->{config}->{ill_avail_api_name} || 'ILL availability - Koha';
};
...
sub ill_availability_services {
    ...
    if ($self->{config}->{"ill_avail_api_display_${ui_context}"}) {
        $can_service_context++;
    }
    ...
    my $endpoint = '/api/v1/contrib/' . $self->api_namespace .
        '/ill_availability_search_api?metadata=';
...
sub configure {
    ...
    unless ( $cgi->param('save') ) {
        ...
        print $cgi->redirect(-url => '/cgi-bin/koha/plugins/run.pl?class=Koha::Plugin::Com::PTFSEurope::AvailabilityApi&method=configure');
        exit;
    }
}
